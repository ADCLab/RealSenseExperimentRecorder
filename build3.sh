...
if [[ -f "$OUTPUT_EXECUTABLE" ]]; then
    ...
    echo "Exec=sudo env DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY ${DIST_EXPERIMENT_FOLDER}/${EXPERIMENT_NAME}_${EXPERIMENT_TAG}" >> "$DESKTOP_SHORTCUT"
    echo "Icon=${IMAGE_ASSET}" >> "$DESKTOP_SHORTCUT"
    echo "Terminal=true" >> "$DESKTOP_SHORTCUT" # Set Terminal to true for sudo password prompt
