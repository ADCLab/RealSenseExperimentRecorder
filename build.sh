...
if [[ -z "$EXPERIMENT_NAME" || -z "$EXPERIMENT_TAG" || -z "$IMAGE_ASSET" || -z "$DIST_FOLDER" || -z "$BUILD_FOLDER" ]]; then
    echo "One or more required environment variables are missing in $ENV_FILE."
    exit 1
fi

# Create a dynamically named distribution folder
DIST_EXPERIMENT_FOLDER="${DIST_FOLDER}_${EXPERIMENT_NAME}_${EXPERIMENT_TAG}"
